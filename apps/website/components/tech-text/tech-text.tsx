...
export const TechText: React.FC<TechTextProps> = ({
  ...
  const effectiveColor = color ?? (isDark ? "#fafafa" : "#09090b");
  const effectiveAccentColor = accentColor ?? (isDark ? "#ffffff" : "#000000");
  ...
  useEffect(() => {
    ...
    const fontFor = (s: any, size: number) =>
    ...
    const setFont = (
      ...
      if ("letterSpacing" in target) {
        (target as any).letterSpacing = `${s.letterSpacing * size}px`;
      }
      target.textAlign = "left";
      target.textBaseline = "alphabetic";
    };

    const computeTextMetrics = (s: any, availableWidth: number) => {
      ...
      // Scale to fit available width perfectly so x-axis touches borders with 0 padding
      let fit = availableWidth / rawWidth;
      let size = s.fontSize * fit;

      setFont(probe, s, size);
      m = probe.measureText(s.text);
      let inkWidth = Math.max(
        m.actualBoundingBoxLeft + m.actualBoundingBoxRight,
        1,
      );

      if (Math.abs(inkWidth - availableWidth) > 0.5) {
        fit = availableWidth / inkWidth;
        size = size * fit;
        setFont(probe, s, size);
        m = probe.measureText(s.text);
        inkWidth = Math.max(
      ...
      return { size, inkWidth, inkHeight, m };
    };
    ...
    const ensureLayout = (s: any) => {
      ...
      const probe = scratchCtx;
      const { size, inkWidth, inkHeight, m } = computeTextMetrics(s, width);

      // Zero padding on all sides:
      // x = m.actualBoundingBoxLeft makes leftmost ink touch x = 0
      // baseline = m.actualBoundingBoxAscent makes topmost ink touch y = 0
      const x = m.actualBoundingBoxLeft;
      const baseline = m.actualBoundingBoxAscent;

      const next: LayoutWord = {
        size,
        baseline,
        left: 0,
        right: inkWidth,
    ...
    const falloff = (
      ...
      const inner = Math.min(1, Math.max(0, 1 - softnessVal));
    ...
    const drawSpecks = (s: any, a: number) => {
    ...
    const drawFrame = (s: any) => {
      ...
      if (frame.y1 < 14) {
        ...
        ctx.fillText(label, Math.round(frame.x1) + 2, Math.round(frame.y1) + 2);
      } else {
    ...
    const resize = () => {
      const s = settingsRef.current;
      if (!s || !container || !canvas) return;

      const currentWidth = Math.max(1, Math.round(container.clientWidth));
      if (currentWidth === lastWidth && height > 1) return;
      lastWidth = currentWidth;

      const { inkHeight } = computeTextMetrics(s, currentWidth);

      width = currentWidth;
      height = Math.max(1, Math.round(inkHeight));
